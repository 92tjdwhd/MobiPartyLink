...
serve(async (req) => {
  ...
  try {
    const requestBody: FCMRequest = await req.json();
    ...
    if (requestBody.type && requestBody.table) {
      ...
      if (!fcm_tokens || fcm_tokens.length === 0) {
        return new Response(
          JSON.stringify({ error: 'fcm_tokens required' }),
...
// Webhook 처리 함수
async function handleWebhook(payload: FCMRequest) {
...
// 파티 테이블 Webhook 처리
async function handlePartyWebhook(type: string, record: any, old_record: any) {
  ...
  return await sendFCM(fcmTokens, title, body, { type: eventType, party_name: partyName });
}

// 파티 멤버 테이블 Webhook 처리 (강퇴 또는 파티 삭제)
async function handlePartyMemberWebhook(type: string, record: any, old_record: any) {
  if (type !== 'DELETE') {
    return new Response(
      JSON.stringify({ success: true, message: 'Not a DELETE event' }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
  ...
  if (!party || error) {
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'Party deleted, skip notification' }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } else {
    // 강퇴됨
    ...
    return await sendFCM(
      [fcmToken],
      '파티에서 강퇴됨',
      `[${party.name}] 파티에서 강퇴되었습니다`,
      { type: 'member_kicked', party_name: party.name }
    );
  }
}
...
async function sendFCM(
  ...
  console.log('📩 FCM 전송 시작:', { tokens: fcmTokens.length, title });
